...
class ProductGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Product entity")
        ...
        .repeat(2) {
            ...
            .post("/ad/api/products")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "name":"SAMPLE_TEXT", "description":"SAMPLE_TEXT", "publicPrice":"0", "adPrice":"0", "image":null}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_product_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created product")
                .get("/ad${new_product_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created product")
            .delete("/ad${new_product_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
