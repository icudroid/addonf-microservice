...
class CompanyGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Company entity")
        ...
        .repeat(2) {
            ...
            .get("/ad/api/companies")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new company")
            .post("/ad/api/companies")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "name":"SAMPLE_TEXT", "siret":"SAMPLE_TEXT", "siren":"SAMPLE_TEXT", "legalStatus":null, "logo":null}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_company_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created company")
                .get("/ad${new_company_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created company")
            .delete("/ad${new_company_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
