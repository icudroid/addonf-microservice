...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.customerTargetService.search({
                page: this.page - 1,
        ...
        this.customerTargetService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
            (res: ResponseWrapper) => this.onSuccess(res.json, res.headers),
    ...
    transition() {
        this.router.navigate(['/customer-target'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/customer-target', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: CustomerTarget) {
        return item.id;
    }
    registerChangeInCustomerTargets() {
        this.eventSubscriber = this.eventManager.subscribe('customerTargetListModification', (response) => this.loadAll());
    }
